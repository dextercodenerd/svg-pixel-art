...
function toCanvasFillStyle(value: string): string {
...
function drawPixels(
  ...
  for (let row = 0; row < height; row += 1) {
    ...
    for (let col = 0; col < width; col += 1) {
      const pixel = pixels[rowOffset + col]
      if (pixel == null || isTransparentPixel(pixel)) {
        continue
      }

      context.fillStyle = toCanvasFillStyle(pixel)
      context.fillRect(
        col * renderScale.value,
        row * renderScale.value,
        renderScale.value,
        renderScale.value,
      )
    }
...
function drawCanvas() {
  ...
  drawPixels(context, props.document.pixels, props.document.width, props.document.height)

  if (props.previewPixels != null && props.previewPixels.length === props.document.pixels.length) {
    drawPixels(context, props.previewPixels, props.document.width, props.document.height, 0.65)
  }
...
watchEffect(() => {
  void props.document.width
  void props.document.height
  void props.document.pixels
  void props.zoom
