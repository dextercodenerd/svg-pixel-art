...
Follow the existing Vue SFC pattern with `<script setup lang="ts">`. Prettier is the formatting source of truth: 2-space indentation, single quotes, no semicolons, trailing commas, 100-character line width, and LF endings. ESLint enforces additional rules such as mandatory curly braces. Use PascalCase for Vue component filenames (`HelloWorld.vue`), camelCase for variables and functions, and keep component logic, template, and styles grouped in the same `.vue` file when practical.
