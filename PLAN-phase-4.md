...
- [x] 1. Create `src/components/color/FgBgDisplay.vue`:
...
- [x] 2. Create `src/components/color/ActiveColorSlotToggle.vue`:
...
- [x] 6. Create `src/components/color/ColorPicker.vue`:
...
- [x] 9. Wire all color components into the right panel of `EditorShell`.

## Verify

- [x] HSV, RGBA sliders, and hex field stay in sync inside the picker.
- [x] `FG/BG` store values update only after picker confirmation.
- [x] Picker cancel leaves colors unchanged.
- [x] `X` swaps `FG/BG`.
- [x] Active slot toggle controls touch drawing/fill/pick behavior.
- [x] Palette swatches persist across reloads.
- [x] Right-click on a swatch sets `BG`.
- [x] Alpha slider controls transparency.
